...
##########################################################################################################
##########################################################################################################
# script for metabarcoding:  Schirmer et al (2015, NAR) + Zepeda-Mendoza et al (2016) Begum pipeline
##########################################################################################################
###########################################################################################################

# using Begum fork of Dame pipeline: http://github.com/shyamsg/Begum
# $HOMEFOLDER is your folder holding the scripts, data, and output folders

# run this script from ~/PATH_TO/$HOME/scripts
...
BEGUM="/Users/Negorashi2011/src/Begum/src/" # pathname for the binaries on Begum
    ...
    python ${BEGUM}Begum.py sort -h
    python ${BEGUM}Begum.py filter -h
...
#### Use fastp to trim adapters. trim bad nucleotides, and merge reads
# fastp -h # 0.20.0
...
do
      ...
      fastp --merge -i ${sample_prefix}_L001_R1_001.fastq.gz -I ${sample_prefix}_L001_R2_001.fastq.gz --merged_out ${sample_prefix}_mergedtrimmed.fq.gz # -o ${sample_prefix}.R1.fq.gz -O ${sample_prefix}.R2.fq.gz
done
...
do
    ...
    mv ${sample_prefix}_mergedtrimmed.fq.gz folder_${sample_prefix}/
done
...
for sample in "${sample_names[@]}"  # ${sample_names[@]} is the full bash array.  So loop over all samples
do
      sample_prefix="$( basename $sample "_mergedtrimmed.fq.gz")"  # e.g. A1_S1 is a sample_prefix
      cd ${HOMEFOLDER}data/seqs # starting point
      echo ${sample_prefix}
      cd folder_${sample_prefix}
      python ${BEGUM}Begum.py sort -fq sickle_cor_panda_${sample_prefix}.fastq.gz -p ${HOMEFOLDER}data/Primers_COILeray.txt -t ${HOMEFOLDER}data/Tags_BiosoupII_COI.txt
...
# mkdir folder{A,B,C,D,E,F}/pool{1,2,3}
parallel mv {1}{2}_*_mergedtrimmed.fq.gz folder{1}/pool{2} ::: A B C D E F ::: 1 2 3



# #### Use AdapterRemoval to trim away Illumina sequencing adapters. (there are always a few adapters left in the raw data)
# # AdapterRemoval -h
#
...
# done
#
# ## -s sickle_Single${sample_prefix}.fq:  keep single read （not paired but good sequences), will not be used in Begum pipeline as we are using twin tags but can be used in genome research.
#
# #### remove the AdapterRemoval files
# rm Adaptermv*.*
#
# #### SPAdes for error correction, via BayesHammer
# #### not using spades.py --meta because these are amplicon data
# # spades.py -h
#
